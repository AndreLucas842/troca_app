...
module.exports = {
    ...
    login: async (req,res) =>{
        try{
            ...
            //Compara a senha que o usuario digitou, com a senha do usuario retornado no banco   
            const senhaValida = await bcrypt.compare(senha, usuario.senha)
            //Se senhas não coincidirem, mensagem de erro
            if (!senhaValida) return res.status(404).render('erro', {mensagem:"Crendenciais inválidas"})

            //Gera o token de acesso, contendo o perfil
            const token = jwt.sign(
                {id: usuario.id, perfil, nome: usuario.nome},
            ...
            res.status(500).render('erro', {mensagem :"interno no servidor"})
        }
    },
